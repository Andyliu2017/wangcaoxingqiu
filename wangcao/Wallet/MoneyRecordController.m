...
@interface MoneyRecordController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
- (void)getData{
    ...
    [[manager ApiGetIncomeDetail:self.page limit:10] subscribeNext:^(WCBaseModel *model) {
        for (int i = 0; i < model.content.count; i++) {
            MoneyRecordModel *recModel = [MoneyRecordModel mj_objectWithKeyValues:model.content[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.dataArr.count > indexPath.row) {
        [cell setMoneyDetailData:self.dataArr[indexPath.row]];
    }
    return cell;
}
